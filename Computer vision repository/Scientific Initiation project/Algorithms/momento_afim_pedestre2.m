...
Ip = imread('EE_pedestre2.jpg');
Ip = double(Ip);  % Extremamente necessário.
 [m,n] = size(Ip);
  y = [1:m]';
  x = 1:n;
  ...
  for i = 1:m
      for j = 1:n
          ...
          T2(i,j) = Ip(i,j)*y(i,1);
           % Ordem 2:
          T3(i,j) = Ip(i,j)*x(1,j)*y(i,1);
          T4(i,j) = Ip(i,j)*(x(1,j)^2);
          T5(i,j) = Ip(i,j)*(y(i,1)^2);
          % Ordem 3:
          T6(i,j) = y(i,1)*Ip(i,j)*(x(1,j)^2);
          T7(i,j) = (y(i,1)^2)*Ip(i,j)*x(1,j);
          T8(i,j) = Ip(i,j)*(x(1,j)^3);
          T9(i,j) = (y(i,1)^3)*Ip(i,j);
      end
  end
